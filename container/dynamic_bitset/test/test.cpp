...
{
  nop::container::dynamic_bitset<> testVector{filledVector};
...
{
  EXPECT_EQ(nop::details::dynamic_bitset_limits::MAX_SIZE, emptyVector.max_size());
}
...
TEST_F(VECTOR_TEST_FIXTURE, SET_METHOD)
{
  ...
  nop::container::dynamic_bitset<> testVector{16UL};
  testVector.set();
...
{
  nop::container::dynamic_bitset<> testVector;
...
TEST_F(VECTOR_TEST_FIXTURE, TEMPLATE_ALLOCATOR_TEST)
{
  ...
  nop::container::dynamic_bitset<decltype(pAlloc)> testVector{pAlloc};
